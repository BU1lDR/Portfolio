...
window.PORTFOLIO = {
  ...
  /* ?v= rides the same number as css/ and js/ — see the long comment above the
     stylesheet <link> in index.html, which documents this file as one of the
     three the bump has to touch. The PDF is regenerated by
     tools/build-resume.js at the same path every time the resume changes, so
     without a version a returning visitor is handed the previous one.

     This is the only place it belongs. Module 12 in main.js rewrites the href of
     every [data-resume], #rvGet and #rvTab link from here, so versioning the
     ...
     JS off. */
  resumeUrl: 'assets/resume.pdf?v=23',
